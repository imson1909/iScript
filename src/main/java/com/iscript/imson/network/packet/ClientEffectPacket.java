...
import com.iscript.imson.client.CameraHandler;
import com.iscript.imson.client.ClientQuestCache;
import com.iscript.imson.gui.screen.ScriptLog;
import com.iscript.imson.client.camera.CameraShake;
import com.iscript.imson.client.camera.CutsceneCameraHandler;
import com.iscript.imson.client.camera.CutsceneCameraMode;
import com.iscript.imson.client.camera.CutscenePath;
import com.iscript.imson.client.camera.Interpolation;
import com.iscript.imson.client.camera.PathType;
...
public class ClientEffectPacket {
    ...
    public static void handle(ClientEffectPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            switch (packet.type) {
                ...
                case LOG_MESSAGE -> {
                    String msg = packet.data.getString("Message");
                    String levelStr = packet.data.getString("Level");
                    String sourceFile = packet.data.contains("SourceFile") ? packet.data.getString("SourceFile") : null;
                    int sourceLine = packet.data.contains("SourceLine") ? packet.data.getInt("SourceLine") : -1;
                    ScriptLog.Level level;
                    try { level = ScriptLog.Level.valueOf(levelStr); } catch (Exception e) { level = ScriptLog.Level.INFO; }
                    ScriptLog.get().add(msg, level, sourceFile, sourceLine);
                }
    ...
    public static CompoundTag logMessageToTag(String message, String level, String sourceFile, int sourceLine) {
        CompoundTag data = new CompoundTag();
        data.putString("Message", message);
        data.putString("Level", level);
        if (sourceFile != null) data.putString("SourceFile", sourceFile);
        if (sourceLine >= 0) data.putInt("SourceLine", sourceLine);
        return data;
    }
