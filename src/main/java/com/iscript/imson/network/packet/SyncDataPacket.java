...
import com.iscript.imson.gui.screen.ListSubScreen.NPC;
import com.iscript.imson.gui.screen.ListSubScreen.State;
...
public class SyncDataPacket {
    public enum Type {
        ...
        STATES, DASHBOARD_LIST, STATE_GRAPHS, TRIGGERS
    }
    ...
    public static void handle(SyncDataPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            switch (packet.type) {
                ...
                case STATES -> {
                    if (Minecraft.getInstance().screen instanceof DashboardScreen ds && ds.currentSubScreen instanceof State sub) {
    ...
    private static void handleNPCList(CompoundTag data) {
        ...
        com.iscript.imson.data.NPCManager.updateClientCache(list);
        if (Minecraft.getInstance().screen instanceof DashboardScreen ds && ds.currentSubScreen instanceof NPC sub) {
