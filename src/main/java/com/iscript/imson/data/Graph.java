package com.iscript.imson.data;
...
public class Graph implements DataObject {
    ...
    public boolean connect(String sourceId, Node.Connection connection) {
        Node source = nodes.get(sourceId);
        if (source == null || connection == null) return false;
        source.getConnections().add(connection);
        return true;
    }

    public boolean disconnect(String sourceId, Node.Connection connection) {
        Node source = nodes.get(sourceId);
        if (source == null || connection == null) return false;
        return source.getConnections().remove(connection);
    }
