...
import com.iscript.imson.gui.widget.StyledButton;
...
public class NPC extends DashboardScreen.SubScreen {
    ...
    protected void init() {
        ...
        life.search().request(searchX, searchY, searchW, 18, I18n.t("iscript.npc.list.search"));
        life.init();
    ...
    public void render(GuiGraphics g, int mx, int my, float pt, int x, int y, int w, int h) {
        ...
        for (int i = life.selection().scroll(); i < Math.min(life.selection().scroll() + maxVisible, filtered.size()); i++) {
    ...
    public boolean mouseScrolled(double mx, double my, double delta) {
        ...
        if (delta > 0) {
            life.selection().scroll(Math.max(0, life.selection().scroll() - 1));
        } else {
            life.selection().scroll(Math.min(life.selection().scroll() + 1, maxScroll));
        }
        return true;
    }
