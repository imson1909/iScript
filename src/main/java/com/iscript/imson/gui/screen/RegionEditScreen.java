...
import com.iscript.imson.network.packet.*;
...
public class RegionEditScreen extends Screen {
    ...
    private static final int CONTENT_TOP = 10 + HEADER_H;
    ...
    protected void init() {
        ...
        lbl(I18n.s("iscript.region.edit.title"), 0, fy, true);
        fy += 24;

        fixedBtn(fx, fy, 64, 20, worldMode ? I18n.s("iscript.region.edit.mode_world") : I18n.s("iscript.region.edit.mode_local"), () -> { worldMode = !worldMode; rebuildWidgets(); }, false);
        fixedBtn(fx + 70, fy, 64, 20, sizeMode ? I18n.s("iscript.region.edit.mode_size") : I18n.s("iscript.region.edit.mode_corners"), () -> { sizeMode = !sizeMode; rebuildWidgets(); }, false);

        int x = PANEL_X + 14;
        int y = CONTENT_TOP - scrollOffset;
        int w = PANEL_W - 28;

        lbl(I18n.s("iscript.region.edit.label.name"), x, y, false);
        y += 14;
        ...
        lbl(I18n.s("iscript.region.edit.label.tick_interval"), x, y, false);
        y += 14;
        tickBox = box(x, y, w, String.valueOf(data.getTickInterval()));
        y += 30;

        lbl(I18n.s("iscript.region.edit.label.faction"), x, y, false);
        y += 14;
        ...
        contentHeight = Math.max(0, y - (CONTENT_TOP - scrollOffset));
    }

    private void lbl(String t, int x, int y, boolean fixed) {
        labels.add(new Label(t, x, y, fixed));
    }
    ...
    private Btn fixedBtn(int x, int y, int w, int h, String t, Runnable r, boolean a) {
    ...
    private Btn btn(int x, int y, int w, int h, String t, Runnable r, boolean a) {
        Btn b = new Btn(x, y, w, h, t, r, a);
        scrollableWidgets.add(b);
        addRenderableWidget(b);
        return b;
    }

    private int sizeMode(int x, int y, int w) {
        lbl(I18n.s("iscript.region.edit.label.center"), x, y, false);
        y += 14;
        int cw = (w - 60) / 3;
        Vec3 c = getCenter();
        ...
        y += 24;
        lbl(I18n.s("iscript.region.edit.label.x"), x + 4, y, false);
        lbl(I18n.s("iscript.region.edit.label.y"), x + cw + 10, y, false);
        lbl(I18n.s("iscript.region.edit.label.z"), x + cw * 2 + 16, y, false);
        y += 14;
        btn(x, y, w, 20, I18n.s("iscript.region.edit.button.set_center"), this::setCenterFromPlayer, false);
        y += 26;

        lbl(I18n.s("iscript.region.edit.label.size"), x, y, false);
        y += 14;
        Vec3 s = getSize();
        sxBox = coord(x, y, s.x);
    ...
    private int cornersMode(int x, int y, int w) {
        lbl(I18n.s("iscript.region.edit.label.corner1"), x, y, false);
        y += 14;
        int cw = (w - 60) / 3;
        ...
        lbl(I18n.s("iscript.region.edit.label.corner2"), x, y, false);
        y += 14;
    ...
    private void addScrollable(EditBox b) {
        scrollableWidgets.add(b);
        addRenderableWidget(b);
    }

    private int effects(int x, int y, int w) {
        y = effectSection(x, y, w, I18n.s("iscript.region.edit.effects.enter"), RegionTrigger.TriggerType.ENTER);
        y = effectSection(x, y, w, I18n.s("iscript.region.edit.effects.exit"), RegionTrigger.TriggerType.EXIT);
        y = effectSection(x, y, w, I18n.s("iscript.region.edit.effects.tick"), RegionTrigger.TriggerType.TICK);
        return y;
    }

    private int effectSection(int x, int y, int w, String title, RegionTrigger.TriggerType type) {
        lbl(title, x, y, false);
        y += 14;
        for (int i = 0; i < data.getTriggers().size(); i++) {
            RegionTrigger t = data.getTriggers().get(i);
            if (t.getTriggerType() != type) continue;
            y = effectRow(x, y, t, i);
        }
        btn(x, y, 76, 20, I18n.s("iscript.region.edit.button.add"), () -> {
            RegionTrigger nt = new RegionTrigger();
            nt.setTriggerType(type);
            data.getTriggers().add(nt);
            rebuildWidgets();
        }, true);
        return y + 24;
    }

    private int effectRow(int x, int y, RegionTrigger t, int globalIndex) {
        lbl(I18n.s("iscript.region.edit.label.type"), x, y + 5, false);
        lbl(I18n.s("iscript.region.edit.label.value"), x + 96, y + 5, false);
        EditBox typeBox = new EditBox(this.font, x + 36, y, 58, 18, Component.literal(""));
        typeBox.setValue(t.getEffect().getType().name());
        typeBox.setTextColor(Theme.TEXT_DIM);
        typeBox.setBordered(false);
        addScrollable(typeBox);
        EditBox valBox = new EditBox(this.font, x + 96, y, 140, 18, Component.literal(""));
        valBox.setValue(t.getEffect().getValue());
        valBox.setTextColor(Theme.TEXT);
        valBox.setBordered(false);
        addScrollable(valBox);
        btn(x + 240, y, 20, 18, I18n.s("iscript.region.edit.button.remove"), () -> {
            data.getTriggers().remove(globalIndex);
            rebuildWidgets();
        }, false);
        effectRows.add(new EffectRow(typeBox, valBox, globalIndex));
        return y + 22;
    }
    ...
    private void save() {
        ...
        for (EffectRow row : effectRows) {
            RegionTrigger t = data.getTriggers().get(row.globalIndex);
            try { t.getEffect().setType(RegionEffect.EffectType.valueOf(row.typeBox.getValue().toUpperCase())); } catch (IllegalArgumentException ignored) {}
            t.getEffect().setValue(row.valBox.getValue());
        }
    ...
    public void tick() {
        super.tick();
        if (++autoSaveTicks >= AUTO_SAVE_INTERVAL) {
            autoSaveTicks = 0;
            save();
        }
    }
    ...
    public void render(GuiGraphics g, int mx, int my, float pt) {
        ...
        UI.panel(g, PANEL_X, 10, PANEL_W, this.height - 20);

        for (Label l : labels) {
            if (l.fixed) {
                if (l.text.equals(I18n.s("iscript.region.edit.title"))) {
                    int tw = this.font.width(l.text);
                    g.drawString(this.font, l.text, PANEL_X + (PANEL_W - tw) / 2, l.y, Theme.ACCENT);
                } else {
        ...
        if (minecraft != null) {
            ...
            int scissorX = (int) ((PANEL_X + 1) * scale);
            int scissorY = (int) ((CONTENT_TOP) * scale);
            int scissorW = (int) ((PANEL_W - 2) * scale);
            int scissorH = (int) ((this.height - CONTENT_TOP - 10) * scale);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        if (mouseX >= PANEL_X && mouseX <= PANEL_X + PANEL_W && mouseY >= CONTENT_TOP && mouseY <= this.height - 10) {
            int viewHeight = this.height - CONTENT_TOP - 10;
            int maxScroll = Math.max(0, contentHeight - viewHeight);
            if (delta > 0) scrollOffset = Math.max(0, scrollOffset - 15);
            else scrollOffset = Math.min(scrollOffset + 15, maxScroll);
            rebuildWidgets();
            return true;
        }
        return false;
    ...
    private record Label(String text, int x, int y, boolean fixed) {}
    private record EffectRow(EditBox typeBox, EditBox valBox, int globalIndex) {}
