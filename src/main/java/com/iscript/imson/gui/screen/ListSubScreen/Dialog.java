...
public class Dialog extends DashboardScreen.SubScreen {
    ...
    public void init() {
        ...
        if (editingNodeId != null && currentDialog != null) {
            editingNode = currentDialog.getNode(editingNodeId);
        }
        ...
        life.search().request(rightX + 4, y + 4, RIGHT_PANEL_WIDTH - 8, 16, Component.translatable("iscript.dialog.list.search"));
        ...
        life.modals().register("name",
                ...
                () -> {
                    ...
                    String initial = (mode.equals("rename") && oldId != null) ? oldId : "";
                    life.editors().addBox("name", cx - 100, dy + 20, 200, 20, Component.translatable("iscript.dialog.editor.name"), initial);
        ...
        life.modals().register("nodeEditor",
                ...
                () -> {
                    ...
                    int cx = DashboardScreen.SIDEBAR_W + (this.parent.width - DashboardScreen.SIDEBAR_W) / 2;
                    int dy = this.parent.height / 2 - 70;
                    life.editors().addBox("nodeTitle", cx, dy + 30, 180, 18, Component.translatable("iscript.dialog.node.title"), editingNode.getParam("title"));
                    life.editors().addBox("nodeText", cx, dy + 64, 180, 18, Component.translatable("iscript.dialog.node.text"), editingNode.getParam("text"));
                    life.editors().addBox("nodePortrait", cx, dy + 98, 180, 18, Component.translatable("iscript.dialog.node.portrait"), editingNode.getParam("portrait"));
                    life.editors().addBox("nodeTarget", cx, dy + 132, 180, 18, Component.translatable("iscript.dialog.node.target_dialog"), editingNode.getParam("target_dialog"));
                    parent.setFocusedWidget(life.editors().box("nodeTitle"));
                },
                () -> {
                    if (editingNode != null) {
                        EditBox titleBox = life.editors().box("nodeTitle");
                        EditBox textBox = life.editors().box("nodeText");
                        EditBox portraitBox = life.editors().box("nodePortrait");
                        EditBox targetBox = life.editors().box("nodeTarget");
                        if (titleBox != null) editingNode.setParam("title", titleBox.getValue());
                        if (textBox != null) editingNode.setParam("text", textBox.getValue());
                        if (portraitBox != null) editingNode.setParam("portrait", portraitBox.getValue());
                        if (targetBox != null) editingNode.setParam("target_dialog", targetBox.getValue());
                        saveDialog();
                    }
                    life.editors().remove("nodeTitle");
                    life.editors().remove("nodeText");
                    life.editors().remove("nodePortrait");
                    life.editors().remove("nodeTarget");
                    editingNode = null;
                    life.state().modalFieldValues.remove("editingNodeId");
                },
                "nodeTitle", "nodeText", "nodePortrait", "nodeTarget");
    ...
    private void switchToDialog(String id) {
        ...
        life.canvas().x(0);
        life.canvas().y(0);
        life.canvas().zoom(1.0);
        spawnOffset = 0;
    }

    private void runDialog() {
        if (life.selection().get() == null || currentDialog == null) return;
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_DIALOG, ServerCommandPacket.requestDialogToTag(life.selection().get())));
    }
    ...
    private void addNode(String type) {
        ...
        switch (type) {
            case "start" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.start").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.start_text").getString());
            }
            case "npc" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.npc").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.npc_text").getString());
            }
            case "player" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.player").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.player_text").getString());
            }
            case "choice" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.choice").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.choice_text").getString());
            }
            case "condition" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.condition").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.condition_text").getString());
            }
            case "action" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.action").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.action_text").getString());
            }
            case "jump" -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.jump").getString());
                node.setParam("text", "");
                node.setParam("target_dialog", "");
            }
            default -> {
                node.setParam("title", Component.translatable("iscript.dialog.node.type.start").getString());
                node.setParam("text", Component.translatable("iscript.dialog.node.default.text").getString());
            }
        }
    ...
    private void executeConfirm() {
        ...
        if ("delete".equals(action) && confirmId != null) {
            DataAccess.removeDialogGraph(confirmId);
    ...
    private void renderNode(GuiGraphics graphics, Node node, int leftX, int leftY, int mouseX, int mouseY) {
        ...
        String nodeType = node.getParam("type");
        int headerColor;
        if ("jump".equals(nodeType)) {
            headerColor = 0xFFAA66CC;
        } else {
            headerColor = isStart ? Theme.ACCENT : Theme.BG_INNER;
        }
        ...
        if ("jump".equals(nodeType) && nh > 30) {
            String target = node.getParam("target_dialog");
            if (target != null && !target.isEmpty()) {
                String label = "-> " + font.plainSubstrByWidth(target, nw - 8);
                graphics.drawString(font, label, nx + 4, ny + headerH + 4, Theme.ACCENT);
            }
        } else if (nh > 30) {
        ...
        for (int i = 0; i < outputs; i++) {
            int slotY = ny + getOutputSlotY(nh, headerH, i, outputs);
            int outX = nx + nw;
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick, int x, int y, int w, int h) {
        ...
        if (!life.modals().isOpen("name") && !life.modals().isOpen("nodeEditor") && !life.modals().isOpen("confirm")) {
            ...
            graphics.drawString(this.font, Component.translatable("iscript.dialog.list.title").getString(), rightX + 8, y + 26, Theme.ACCENT);
            ...
            graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.list.new").getString(), rightX + RIGHT_PANEL_WIDTH / 2, newY + 6, Theme.ACCENT);
            if (life.selection().get() != null && currentDialog != null) {
                ...
                graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.list.empty").getString(), leftX + leftW / 2, y + h / 2, Theme.TEXT_MUTE);
            }
        } else {
            graphics.fill(x, y, x + w, y + h, Theme.BG_INNER);
        }
        ...
        if (life.modals().isOpen("name")) {
            ...
            String mode = life.state().modalFieldValues.getOrDefault("nameMode", "create");
            graphics.drawCenteredString(this.font, Component.translatable(mode.equals("rename") ? "iscript.dialog.editor.rename" : "iscript.dialog.editor.new").getString(), cx, dy + 6, Theme.ACCENT);
            EditBox nameInputBox = life.editors().box("name");
            ...
            graphics.drawCenteredString(this.font, Component.translatable(mode.equals("rename") ? "iscript.dialog.editor.rename_btn" : "iscript.dialog.editor.create").getString(), cx - 26, dy + 57, okHovered ? Theme.ACCENT : Theme.TEXT_DIM);
            ...
            graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.editor.cancel").getString(), cx + 26, dy + 57, cancelHovered ? Theme.ERROR : Theme.TEXT_DIM);
        }
        ...
        if (life.modals().isOpen("confirm")) {
            ...
            String confirmId = life.state().modalFieldValues.get("confirmId");
            graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.editor.delete_confirm", confirmId).getString(), cx, dy + 8, Theme.ERROR);
            ...
            graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.editor.delete").getString(), cx - 26, dy + 43, okHovered ? Theme.ERROR : Theme.TEXT_DIM);
            ...
            graphics.drawCenteredString(this.font, Component.translatable("iscript.dialog.editor.cancel").getString(), cx + 26, dy + 43, cancelHovered ? Theme.TEXT : Theme.TEXT_DIM);
        }
        ...
        if (life.modals().isOpen("nodeEditor") && editingNode != null) {
            ...
            int dx = cx - 100;
            int dy = this.parent.height / 2 - 70;
            boolean isJump = "jump".equals(editingNode.getParam("type"));
            int panelH = isJump ? 165 : 135;
            UI.panel(graphics, dx - 10, dy - 10, 210, panelH);
            graphics.renderOutline(dx - 10, dy - 10, 210, panelH, Theme.BORDER_ACCENT);
            graphics.drawString(font, Component.translatable("iscript.dialog.node.edit_title").getString(), dx, dy, Theme.TEXT);
            graphics.drawString(font, Component.translatable("iscript.dialog.node.title").getString(), dx, dy + 18, Theme.TEXT_DIM);
            EditBox nodeEditTitleBox = life.editors().box("nodeTitle");
            EditBox nodeEditTextBox = life.editors().box("nodeText");
            EditBox nodeEditPortraitBox = life.editors().box("nodePortrait");
            EditBox nodeEditTargetDialogBox = life.editors().box("nodeTarget");
            if (nodeEditTitleBox != null) { nodeEditTitleBox.setX(dx); nodeEditTitleBox.setY(dy + 30); nodeEditTitleBox.setVisible(true); }
            graphics.drawString(font, Component.translatable("iscript.dialog.node.text").getString(), dx, dy + 52, Theme.TEXT_DIM);
            if (nodeEditTextBox != null) { nodeEditTextBox.setX(dx); nodeEditTextBox.setY(dy + 64); nodeEditTextBox.setVisible(true); }
            graphics.drawString(font, Component.translatable("iscript.dialog.node.portrait").getString(), dx, dy + 86, Theme.TEXT_DIM);
            if (nodeEditPortraitBox != null) { nodeEditPortraitBox.setX(dx); nodeEditPortraitBox.setY(dy + 98); nodeEditPortraitBox.setVisible(true); }
            if (isJump && nodeEditTargetDialogBox != null) {
                graphics.drawString(font, Component.translatable("iscript.dialog.node.target_dialog").getString(), dx, dy + 120, Theme.TEXT_DIM);
                nodeEditTargetDialogBox.setX(dx);
                nodeEditTargetDialogBox.setY(dy + 132);
                nodeEditTargetDialogBox.setVisible(true);
            }
        }

        if (showContextMenu && contextMenuNodeId != null) {
            Node ctxNode = currentDialog != null ? currentDialog.getNode(contextMenuNodeId) : null;
            boolean ctxIsJump = ctxNode != null && "jump".equals(ctxNode.getParam("type"));
            int wctx = 100;
            int hctx = ctxIsJump ? 110 : 88;
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.node.edit").getString(), contextMenuX + 4, cy + 6, h1 ? Theme.TEXT : Theme.TEXT_DIM);
            cy += 22;
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.node.set_start").getString(), contextMenuX + 4, cy + 6, h2 ? Theme.TEXT : Theme.TEXT_DIM);
            cy += 22;
            if (ctxIsJump) {
                ...
                graphics.drawString(font, Component.translatable("iscript.dialog.node.follow_jump").getString(), contextMenuX + 4, cy + 6, hJump ? Theme.ACCENT : Theme.TEXT_DIM);
                cy += 22;
            }
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.node.clear_connections").getString(), contextMenuX + 4, cy + 6, h3 ? Theme.ERROR : Theme.TEXT_DIM);
            cy += 22;
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.node.delete").getString(), contextMenuX + 4, cy + 6, h4 ? Theme.ERROR : Theme.TEXT_DIM);
        }
        ...
        if (showConnectionContextMenu && connectionContextFromId != null) {
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.connection.delete").getString(), connectionContextMenuX + 4, connectionContextMenuY + 6, h1 ? Theme.ERROR : Theme.TEXT_DIM);
        }
    ...
    private void renderNodeTypeMenu(GuiGraphics graphics, int mouseX, int mouseY) {
        String[] types = {"start", "npc", "player", "choice", "condition", "action", "jump"};
        int w = 110;
        int h = types.length * 22 + 4;
        graphics.fill(nodeTypeMenuX - 2, nodeTypeMenuY - 2, nodeTypeMenuX + w + 2, nodeTypeMenuY + h + 2, Theme.BG_PANEL);
        ...
        for (String type : types) {
            ...
            graphics.drawString(font, Component.translatable("iscript.dialog.node.type." + type).getString(), nodeTypeMenuX + 6, cy + 6, hovered ? Theme.TEXT : Theme.TEXT_DIM);
            cy += 22;
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (life.search().box() != null && life.search().box().mouseClicked(mouseX, mouseY, button)) {
        ...
        if (life.modals().isOpen("nodeEditor")) {
            ...
            EditBox nodeEditPortraitBox = life.editors().box("nodePortrait");
            EditBox nodeEditTargetDialogBox = life.editors().box("nodeTarget");
            if (mouseX >= dx - 10 && mouseX <= dx + 200 && mouseY >= dy - 10 && mouseY <= dy + 155) {
                ...
                if (nodeEditTargetDialogBox != null && mouseX >= nodeEditTargetDialogBox.getX() && mouseX <= nodeEditTargetDialogBox.getX() + nodeEditTargetDialogBox.getWidth() && mouseY >= nodeEditTargetDialogBox.getY() && mouseY <= nodeEditTargetDialogBox.getY() + nodeEditTargetDialogBox.getHeight()) {
                    parent.setFocusedWidget(nodeEditTargetDialogBox);
                    return nodeEditTargetDialogBox.mouseClicked(mouseX, mouseY, button);
                }
                return true;
            }
            closeNodeEditor();
        ...
        if (showContextMenu) {
            ...
            if (mouseX >= contextMenuX && mouseX <= contextMenuX + 100) {
                ...
                y += 22;
                Node ctxNode = currentDialog != null ? currentDialog.getNode(contextMenuNodeId) : null;
                if (ctxNode != null && "jump".equals(ctxNode.getParam("type"))) {
                    if (mouseY >= y && mouseY <= y + 20) {
                        String target = ctxNode.getParam("target_dialog");
                        if (target != null && !target.isEmpty() && DataAccess.dialogGraph(target) != null) {
                            switchToDialog(target);
                        }
                        showContextMenu = false;
                        return true;
                    }
            ...
            if (button == 0) {
                ...
                for (int i = nodes.size() - 1; i >= 0; i--) {
                    ...
                    for (int j = 0; j < outputs; j++) {
                        int slotY = ny + getOutputSlotY(nh, headerH, j, outputs);
                        int outX = nx + nw;
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        ...
        if (life.modals().isOpen("nodeEditor")) {
            ...
            if (nodeEditTargetDialogBox != null && nodeEditTargetDialogBox.isFocused()) {
                return nodeEditTargetDialogBox.charTyped(codePoint, modifiers);
            }
            return true;
        }
        return false;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (life.modals().isOpen("nodeEditor")) {
            ...
            if (nodeEditTargetDialogBox != null && nodeEditTargetDialogBox.isFocused()) {
                return nodeEditTargetDialogBox.keyPressed(keyCode, scanCode, modifiers);
            }
            return true;
        }
        if ((keyCode == 259 || keyCode == 261) && selectedNodeId != null && currentDialog != null && !life.modals().isOpen("name") && !life.modals().isOpen("nodeEditor") && !life.modals().isOpen("confirm") && !showContextMenu && !showNodeTypeMenu && !showConnectionContextMenu && !contextMenu.isOpen()) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (currentDialog != null && mouseX >= leftX && mouseX <= toolbarX && mouseY >= leftY && mouseY <= leftY + leftH) {
            ...
            life.canvas().zoom(Math.max(0.3, Math.min(3.0, life.canvas().zoom())));
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (life.modals().isOpen("name") || life.modals().isOpen("confirm") || showContextMenu || life.modals().isOpen("nodeEditor") || showNodeTypeMenu || showConnectionContextMenu || contextMenu.isOpen()) return true;
        if (button == 0) {
            if (draggingNodeId != null && currentDialog != null) {
                Node node = currentDialog.getNode(draggingNodeId);
                if (node != null) {
                    int grid = 20;
                    node.setX(Math.round(node.getX() / (float) grid) * grid);
                    node.setY(Math.round(node.getY() / (float) grid) * grid);
                    if (node.getX() != dragStartNodeX || node.getY() != dragStartNodeY) {
                        saveDialog();
                    }
                }
            }
