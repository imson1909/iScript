...
public class Log extends DashboardScreen.SubScreen {
    ...
    public Log(DashboardScreen parent) {
        super(parent);
    }
    ...
    public void render(GuiGraphics g, int mx, int my, float pt, int x, int y, int w, int h) {
        ...
        if (life.selection().scroll() > maxScroll) life.selection().scroll(maxScroll);

        if (selectedIndex >= total) selectedIndex = total - 1;
        if (selectedIndex < -1) selectedIndex = -1;

        for (int i = life.selection().scroll(); i < Math.min(life.selection().scroll() + maxVisible, total); i++) {
            var e = entries.get(i);
