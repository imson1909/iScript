...
import com.iscript.iscript.state.StateMachineManager;
...
public class StateListSubScreen extends DashboardScreen.SubScreen {
    ...
    private static final int TAB_NODES = 0;
    private static final int TAB_VARIABLES = 1;
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick, int x, int y, int w, int h) {
        if (!showNameDialog && !showConfirmDialog) {
            ...
            boolean graphHovered = mouseX >= toolbarX + 4 && mouseX <= toolbarX + TOOLBAR_WIDTH - 4 && mouseY >= btnY && mouseY <= btnY + btnSize;
            graphics.fill(toolbarX + 4, btnY, toolbarX + TOOLBAR_WIDTH - 4, btnY + btnSize, graphHovered ? 0xFF2A3A4A : 0xFF1E1E28);
            graphics.renderOutline(toolbarX + 4, btnY, TOOLBAR_WIDTH - 8, btnSize, 0xFF444455);
            graphics.drawCenteredString(this.font, "\u25A0", toolbarX + TOOLBAR_WIDTH / 2, btnY + (btnSize - 8) / 2, graphHovered ? 0xFF55AAFF : 0xFF4488AA);
            ...
            if (selectedId != null && cache.containsKey(selectedId)) {
                if (leftPanelMode == MODE_LOADING) {
                    ...
                    int tabY = leftY + 70;
                    String[] tabs = {"Nodes", "Variables"};
                    int tabW = leftW / tabs.length;
                    for (int i = 0; i < tabs.length; i++) {
                        boolean active = i == editorTab;
                        int tx = leftX + i * tabW;
                        graphics.fill(tx, tabY, tx + tabW, tabY + 20, active ? 0xFF2A2A3A : 0xFF1A1A24);
                        graphics.renderOutline(tx, tabY, tabW, 20, 0xFF2A2A3A);
                        graphics.drawCenteredString(this.font, tabs[i], tx + tabW / 2, tabY + 6, active ? 0xFF55AAFF : 0xFF888899);
                    }
                    ...
                    if (editorTab == TAB_NODES) {
                        graphics.drawCenteredString(this.font, "[Graph Editor Placeholder]", leftX + leftW / 2, contentY + contentH / 2, 0xFF555566);
                    } else {
                        graphics.drawCenteredString(this.font, "[Variables Placeholder]", leftX + leftW / 2, contentY + contentH / 2, 0xFF555566);
                    }
                }
            } else {
                clearEditorWidgets();
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (leftPanelMode == MODE_EDITOR) {
            ...
            for (int i = 0; i < tabs.length; i++) {
                int tx = leftX + i * tabW;
                if (mouseX >= tx && mouseX <= tx + tabW && mouseY >= tabY && mouseY <= tabY + 20) {
                    editorTab = i;
                    return true;
                }
            }
    ...
    private void openGraphEditor() {
        if (editorMachineId == null) return;
        String name = nameEditBox != null ? nameEditBox.getValue() : editorMachineId;
        String entry = entryNodeEditBox != null ? entryNodeEditBox.getValue() : "";
        sendSave();
        parent.currentSubScreen = new StateGraphSubScreen(parent, level, editorMachineId, name);
        parent.currentSubScreen.init(this.minecraft, this.minecraft.getWindow().getGuiScaledWidth(), this.minecraft.getWindow().getGuiScaledHeight());
    }
