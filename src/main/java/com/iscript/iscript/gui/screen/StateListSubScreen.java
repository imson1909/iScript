...
public class StateListSubScreen extends DashboardScreen.SubScreen {
    ...
    public void init() {
        ...
        clearEditorWidgets();
        dirty = false;
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
    }
    ...
    private void doSave() {
        ...
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.SAVE_STATE_MACHINE, ServerCommandPacket.saveStateMachineToTag(editorMachineId, name, entry, "")));
    }
    ...
    private void switchToMachine(String newId) {
        ...
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINE, ServerCommandPacket.requestStateMachineToTag(newId)));
    }
    ...
    private void confirmNameDialog() {
        ...
        if (nameDialogMode.equals("create")) {
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.SAVE_STATE_MACHINE, ServerCommandPacket.saveStateMachineToTag(id, name, "", "")));
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
            switchToMachine(id);
        } else if (nameDialogMode.equals("rename") && renameOldId != null) {
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.RENAME_STATE_MACHINE, ServerCommandPacket.renameStateMachineToTag(renameOldId, id, name)));
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
        }
    }
    ...
    private void executeConfirm() {
        if ("delete".equals(confirmDialogAction) && confirmDialogId != null) {
            ...
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.DELETE_STATE_MACHINE, ServerCommandPacket.deleteStateMachineToTag(confirmDialogId)));
            IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
        }
        closeConfirmDialog();
    }
    ...
    private void pasteItem() {
        ...
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.DUPLICATE_STATE_MACHINE, ServerCommandPacket.dupStateMachineToTag(sourceId, newId)));
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
    ...
    private void duplicateItem(String id) {
        ...
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.DUPLICATE_STATE_MACHINE, ServerCommandPacket.dupStateMachineToTag(id, newId)));
        IScriptNetwork.sendToServer(new ServerCommandPacket(ServerCommandPacket.Type.REQUEST_STATE_MACHINES, new CompoundTag()));
        switchToMachine(newId);
    }
